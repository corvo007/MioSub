...
| 📦 **其他功能**       | 双语 SRT/ASS 导出、版本快照、自定义 API 端点、缓存管理        |
...
**解决的问题**：标准视频播放器无法渲染复杂的字幕样式（ASS），且传统预览方式响应缓慢。

**功能特点**：

- **实时渲染**：内置 `assjs` 引擎，支持字体、颜色、位置等样式的精确还原。
- **智能缓存**：高效缓存转码后的预览片段，确保流畅的进度条切换与播放体验。
- **缓存管理**：专门的 UI 界面，支持监控与一键清理预览缓存，节省磁盘空间。
- **原文切换**：一键切换显示原文或译文，极大提升校对效率。
...
- Gemini 2.0 Flash：并发数 5（速度优先）
- Gemini 2.0 Pro：并发数 2（避免超限）
