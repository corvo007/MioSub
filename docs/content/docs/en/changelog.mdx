...
- **Whisper (Local)**: Detect when a model file (e.g. `.bin`) is mistakenly selected as the Whisper executable and show a clear, actionable error during preflight, instead of failing later with a cryptic spawn error (Fixes MIOSUB-5X).
- **Whisper (Local)**: Retry the binary version/GPU probe once after a brief warm-up on transient cold-start failures, improving the reliability of version and GPU-support detection.
