...
│   │       ├── 📄 EndToEndProgress.tsx # 进度展示
│   │       ├── 📂 wizard/           # 向导组件
│   │       └── 📂 shared/           # 共享组件
│   │
│   ├── 📂 hooks/                    # React Hooks
│   │   ├── 📂 useWorkspaceLogic/    # 核心工作区逻辑 (目录)
│   │   │   └── 📄 index.ts          # 逻辑入口
│   │   ├── 📄 useSettings.ts        # 设置管理
...
│   ├── 📄 preload.ts                # 预加载脚本
│   └── 📂 services/                 # 桌面端服务
│       ├── 📄 localWhisper.ts       # 本地 Whisper
│       ├── 📄 ffmpegAudioExtractor.ts # FFmpeg 音频提取
│       ├── 📄 videoCompressor.ts    # 视频压缩服务
│       ├── 📄 ytdlp.ts              # 视频下载服务
│       ├── 📄 endToEndPipeline.ts   # 全自动流水线
│       └── 📄 storage.ts            # 存储服务
│
├── 📂 resources/                    # 资源文件
│   └── 📄 whisper-cli.exe           # Whisper CLI (Windows)
│
├── 📄 package.json                  # 项目配置
├── 📄 vite.config.ts                # Vite 配置
├── 📄 vite.config.electron.ts       # Electron Vite 配置
