...
export default function App() {
    // View State: 'home' is the entry selection screen, 'workspace' is the editor
    const [view, setView] = useState<'home' | 'workspace'>('home');

    // Logic State
    const [activeTab, setActiveTab] = useState<'new' | 'import'>('new'); // Kept for internal logic distinction
    ...
    // Scroll to bottom on new content if processing
    useEffect(() => {
    ...
    const createSnapshot = (description: string, subtitlesOverride?: SubtitleItem[]) => {
        const newSnapshot: SubtitleSnapshot = {
            ...
            description,
            subtitles: JSON.parse(JSON.stringify(subtitlesOverride || subtitles)), // Deep copy
    ...
    const selectBatchesWithComments = (chunks: SubtitleItem[][]) => {
        ...
        chunks.forEach((chunk, idx) => {
            ...
            if (hasBatchComment || hasLineComments) {
                newSet.add(idx);
            }
        });
        setSelectedBatches(newSet);
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            // If we are in 'new' mode, we might need to clear previous results if any
            // But typically this is called either from empty state or re-upload
            if (activeTab === 'new' && subtitles.length > 0 && status === GenerationStatus.COMPLETED) {
                if (!confirm("This will replace the current file and may require re-generation. Continue?")) {
                    return;
                }
                setSubtitles([]);
    ...
    const handleSubtitleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onload = (ev) => {
                ...
                // Only clear snapshots if this is the initial import
                if (subtitles.length === 0) {
                    setSnapshots([]);
                    setBatchComments({});
                } else {
                    // If user imports subtitles on top of existing ones? Replace.
                    setSnapshots([]);
                    setBatchComments({});
                }
    ...
    const handleBatchAction = async (mode: BatchOperationMode, singleIndex?: number) => {
        ...
        // If we are in Import mode without audio file, we can't do timestamp fixing
        if (mode === 'fix_timestamps' && !file) {
            setError("Cannot fix timestamps without source media file. Please upload video/audio first.");
            return;
        }
        ...
        try {
            ...
            // Clear comments for processed batches
            ...
            // Clear selection if it was a multi-select action
            if (singleIndex === undefined) {
                setSelectedBatches(new Set());
            }

            // Create Snapshot after successful action
            const actionName = mode === 'fix_timestamps' ? 'Fix Time' : mode === 'retranslate' ? 'Retranslate' : 'Proofread';
            const count = indices.length;
            createSnapshot(`${actionName} (${count} segments)`, refined);
    ...
    const updateBatchComment = (chunkIdx: number, comment: string) => {
        setBatchComments(prev => ({ ...prev, [chunkIdx]: comment }));
    };
    ...
    const goBackHome = () => {
        if (subtitles.length > 0) {
            if (!confirm("Go back to home? Unsaved progress will be lost.")) return;
        }
        // Fully reset state
        setView('home');
        setSubtitles([]);
            ...
            return (
                <div className="h-full flex flex-col items-center justify-center text-slate-600 p-8 min-h-[300px]">
                    ...
                    <p className="text-sm mt-2 max-w-xs text-center opacity-70">
                        {activeTab === 'new' ? 'Upload a media file to start.' : 'Import an SRT/ASS file to begin editing.'}
                    </p>
                </div>
            );
        }
                    ...
                    <div className="flex flex-wrap items-center gap-3 bg-slate-800/90 p-3 rounded-lg border border-slate-700 sticky top-0 z-20 backdrop-blur-md shadow-md justify-between">
                        <div className="flex items-center space-x-4">
                            ...
                            <button
                                ...
                                className="flex items-center space-x-2 text-sm text-slate-300 hover:text-white transition-colors"
                                title="Select segments with active comments"
                            >
                                <MessageCircle className="w-4 h-4 text-amber-400" />
                                <span className="hidden sm:inline">Select Commented</span>
                            </button>
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            {file && (
                                <button
                                    ...
                                    title="Fix Timestamps (Audio Required)"
                                    className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-md text-xs font-bold transition-all shadow-sm border ${selectedBatches.size > 0
                            ...
                            <button
                                ...
                                disabled={selectedBatches.size === 0}
                                title="Deep Proofread (Respects Comments)"
                                className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-md text-xs font-bold transition-all shadow-sm border ${selectedBatches.size > 0
                            ...
                            >
                                ...
                                <div className="flex-1 px-2">
                                    ...
                                    <input
                                        ...
                                        placeholder="Add instruction for this whole segment..."
                                            ...
                                            {(editingCommentId === sub.id || sub.comment) && (
                                                <div className="mt-2 flex items-start animate-fade-in">
                                                    ...
                                                    <input
                                                        ...
                                                        placeholder="Add specific correction instruction..."
            ...
            <header className="flex justify-between items-center mb-12">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400">AI-Powered Subtitle Creation & Localization</p>
                    </div>
                </div>
                <button
                    onClick={() => setShowSettings(true)}
                    className="p-3 bg-slate-800/50 hover:bg-slate-800 border border-slate-700 rounded-xl transition-colors group"
                >
            ...
            <main className="flex-1 flex flex-col items-center justify-center max-w-4xl mx-auto w-full">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
                    ...
                    <button
                        ...
                        className="group relative bg-slate-900 border border-slate-800 hover:border-indigo-500/50 hover:bg-slate-800/50 rounded-3xl p-8 transition-all duration-300 shadow-2xl flex flex-col items-center text-center cursor-pointer"
                    >
                        <div className="w-20 h-20 bg-slate-800 group-hover:bg-indigo-500/20 rounded-2xl flex items-center justify-center mb-6 transition-colors">
                            <FileVideo className="w-10 h-10 text-indigo-400 group-hover:scale-110 transition-transform" />
                        </div>
                        <h2 className="text-2xl font-bold text-white mb-3">New Project</h2>
                        <p className="text-slate-400 leading-relaxed">
                            Transcribe & Translate from Video/Audio using Whisper & Gemini.
                        </p>
                    </button>

                    {/* Option 2: Open Subtitles */}
                    <button
                        onClick={startImportProject}
                        className="group relative bg-slate-900 border border-slate-800 hover:border-emerald-500/50 hover:bg-slate-800/50 rounded-3xl p-8 transition-all duration-300 shadow-2xl flex flex-col items-center text-center cursor-pointer"
                    >
                        <div className="w-20 h-20 bg-slate-800 group-hover:bg-emerald-500/20 rounded-2xl flex items-center justify-center mb-6 transition-colors">
                            <FileText className="w-10 h-10 text-emerald-400 group-hover:scale-110 transition-transform" />
                        </div>
                        <h2 className="text-2xl font-bold text-white mb-3">Open Subtitles</h2>
                        <p className="text-slate-400 leading-relaxed mb-4">
                            Import existing .SRT or .ASS files to fix timing, proofread, or re-translate.
                        </p>
                        <div className="flex flex-wrap gap-2 justify-center mt-2">
                            <span className="text-xs px-2 py-1 bg-slate-800 rounded border border-slate-700 text-slate-500">Edit Text</span>
                            <span className="text-xs px-2 py-1 bg-slate-800 rounded border border-slate-700 text-slate-500">+ Video Ref</span>
                        </div>
                    </button>
        ...
        <div className="min-h-screen bg-slate-950 text-slate-200 p-4 md:p-8 flex flex-col">
            <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col space-y-6">
                ...
                <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-800 shrink-0">
                    <div className="flex items-center space-x-4">
                        <button onClick={goBackHome} className="p-2 hover:bg-slate-800 rounded-lg transition-colors text-slate-400 hover:text-white">
                            <ArrowLeft className="w-5 h-5" />
                        </button>
                        <div>
                            <h1 className="text-xl font-bold text-white tracking-tight flex items-center gap-2">
                                {activeTab === 'new' ? 'New Project' : 'Subtitle Editor'}
                                <span className="text-xs font-normal text-slate-500 bg-slate-900 border border-slate-800 px-2 py-0.5 rounded">
                                    {activeTab === 'new' ? 'Generation' : 'Import Mode'}
                                </span>
                            </h1>
                            <p className="text-xs text-slate-400 truncate max-w-[300px]">
                                {file ? file.name : (subtitles.length > 0 ? `${subtitles.length} lines loaded` : 'No file selected')}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <button
                            ...
                            className={`flex items-center space-x-2 px-4 py-2 border rounded-lg transition-colors text-sm font-medium ${snapshots.length > 0 ? 'bg-indigo-900/30 border-indigo-500/50 text-indigo-200' : 'bg-slate-900 border-slate-800 text-slate-600'}`}
                        >
                        ...
                        <button
                            onClick={() => setShowSettings(true)}
                            className="p-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg transition-colors text-sm font-medium group"
                        >
                ...
                <div className="flex-1 flex flex-col lg:grid lg:grid-cols-12 gap-6 min-h-0">
                    ...
                    <div className="lg:col-span-3 lg:h-full overflow-y-auto custom-scrollbar space-y-4">
                        ...
                        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 shadow-sm space-y-4">
                            ...
                            {file ? (
                                <div className="flex items-center p-3 bg-slate-800 rounded-lg border border-slate-700/50">
                                    ...
                                    <div className="overflow-hidden flex-1 min-w-0">
                                        ...
                                        <p className="text-[10px] text-slate-500">{Math.floor(duration / 60)}:{Math.floor(duration % 60).toString().padStart(2, '0')} • {(file.size / (1024 * 1024)).toFixed(1)}MB</p>
                                    </div>
                                    <label className="cursor-pointer p-1.5 hover:bg-slate-700 rounded text-slate-400 hover:text-white transition-colors ml-1" title="Change Source File">
                                        <RefreshCcw className="w-3 h-3" />
                                        <input type="file" accept="video/*,audio/*" onChange={handleFileChange} className="hidden" disabled={isProcessing} />
                                    </label>
                                </div>
                            ) : (
                                // Empty Media State - Show Upload Button
                                    ...
                                    <div className="flex flex-col items-center justify-center py-4">
                                        {activeTab === 'new' ? (
                                            <>
                                                ...
                                                <p className="text-xs font-bold text-slate-300">Upload Video / Audio</p>
                                                <p className="text-[10px] text-slate-500 mt-1">To start transcription</p>
                                            </>
                                        ) : (
                                            <>
                                                <Plus className="w-5 h-5 text-slate-500 group-hover:text-indigo-400 mb-1" />
                                                <p className="text-xs text-slate-500">Attach Media (Optional)</p>
                                            </>
                                        )}
                                    </div>
                            ...
                            {/* Subtitle File Section (Only for Import Mode) */}
                            {activeTab === 'import' && (
                                <div className="pt-2 border-t border-slate-800">
                                    <div className="flex items-center justify-between mb-2">
                                        <h3 className="text-xs font-semibold text-slate-400">Subtitle File</h3>
                                        {subtitles.length > 0 && (
                                            <span className="text-[10px] text-emerald-400 bg-emerald-400/10 px-1.5 py-0.5 rounded">{subtitles.length} lines</span>
                                        )}
                                    </div>
                                    ...
                                    {subtitles.length === 0 ? (
                                        <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-slate-700 rounded-lg bg-slate-800/30 hover:bg-slate-800/50 hover:border-emerald-500/50 cursor-pointer transition-all group">
                                            <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                                <FileText className="w-6 h-6 text-emerald-500 group-hover:text-emerald-400 mb-1 group-hover:scale-110 transition-transform" />
                                                <p className="text-xs font-bold text-slate-300">Import .SRT / .ASS</p>
                                            </div>
                                            <input type="file" accept=".srt,.ass" onChange={handleSubtitleImport} className="hidden" />
                                        </label>
                                    ) : (
                                        <div className="flex items-center p-2 bg-slate-800 rounded border border-slate-700/50">
                                            <FileText className="w-4 h-4 text-emerald-500 mr-2" />
                                            <p className="text-xs text-slate-300 flex-1">Subtitles Loaded</p>
                                            <label className="cursor-pointer p-1 hover:bg-slate-700 rounded text-slate-400 hover:text-white" title="Replace Subtitles">
                                                <RefreshCcw className="w-3 h-3" />
                                                <input type="file" accept=".srt,.ass" onChange={handleSubtitleImport} className="hidden" />
                                            </label>
                                        </div>
                            ...
                            <div className="flex flex-col space-y-2 text-xs text-slate-400 bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
                                <div className="flex items-center justify-between">
                                    <span className="flex items-center text-slate-500"><Monitor className="w-3 h-3 mr-2" /> Model</span>
                                    <span className="font-medium text-slate-300">{settings.transcriptionModel === 'whisper-1' ? 'Whisper' : 'GPT-4o'}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="flex items-center text-slate-500"><Clapperboard className="w-3 h-3 mr-2" /> Genre</span>
                                    <span className="font-medium text-slate-300 truncate max-w-[120px]" title={settings.genre}>{settings.genre}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {(status === GenerationStatus.COMPLETED || status === GenerationStatus.PROOFREADING) && subtitles.length > 0 && (
                            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 shadow-sm animate-fade-in">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <button onClick={() => handleDownload('srt')} className="flex flex-col items-center justify-center p-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-slate-600 rounded-lg transition-all">
                                        <span className="font-bold text-slate-200 text-sm">.SRT</span>
                                    </button>
                                    <button onClick={() => handleDownload('ass')} className="flex flex-col items-center justify-center p-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-slate-600 rounded-lg transition-all">
                                        <span className="font-bold text-slate-200 text-sm">.ASS</span>
                                    </button>
                                </div>
                                <div className="mt-3 text-[10px] text-center text-slate-500">
                                    Mode: {settings.outputMode === 'bilingual' ? 'Bilingual' : 'Translated Only'}
                                </div>
                            </div>
                        )}
                    ...
                    {/* Right Column: Preview & Editor (Expanded) */}
                    <div className="lg:col-span-9 flex flex-col h-[500px] lg:h-full min-h-0">
                        ...
                        <div className="flex items-center justify-between mb-2 h-8 shrink-0">
                            <div className="flex items-center space-x-2">
                                {/* Breadcrumbs or minor info could go here */}
                            </div>
                            <StatusBadge />
                        </div>
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar w-full relative">
                                    ...
                                    {snapshots.length === 0 ? (
                                        <div className="h-full flex flex-col items-center justify-center text-slate-500 opacity-50">
                                            ...
                                            <p>No versions available for this session</p>
                                        </div>
                                    ) : (
                                        snapshots.map((snap) => (
                                            <div key={snap.id} className="bg-slate-800/50 border border-slate-700 p-4 rounded-xl flex justify-between items-center">
                                                <div>
                                                    <h4 className="font-medium text-slate-200">{snap.description}</h4>
                                                    <p className="text-xs text-slate-500 mt-1">{snap.timestamp}</p>
                                                </div>
                                                <button
                                                    onClick={() => restoreSnapshot(snap)}
                                                    className="px-3 py-1.5 bg-slate-700 hover:bg-indigo-600 rounded text-xs text-white transition-colors flex items-center"
                                                >
                ...
                <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in overflow-y-auto">
                    <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-2xl shadow-2xl relative my-12">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        {ENV_GEMINI_KEY && !settings.geminiKey && (
                                        ...
                                        {ENV_GEMINI_KEY && settings.geminiKey && (
                                            <p className="text-xs text-amber-400 mt-1">
                                                Overriding environment API Key
                                            </p>
                                        )}
                                    </div>
                                    <div>
                                        ...
                                        {ENV_OPENAI_KEY && !settings.openaiKey && (
                                            <p className="text-xs text-emerald-400 mt-1 flex items-center">
                                                <CheckCircle className="w-3 h-3 mr-1" />
                                                Using API Key from environment
                                            </p>
                            ...
                            {/* Model & Genre Section - Redesigned for Custom Input */}
                            <div className="space-y-3 pt-4 border-t border-slate-800">
                                ...
                                {/* Custom Genre Input - Full Width Row */}
                                {isCustomGenre && (
                                    <div className="pt-2 animate-fade-in">
                                        <label className="block text-xs font-medium text-indigo-400 mb-1.5">Custom Context / Genre Description</label>
                                        <input
                                            type="text"
                                            value={settings.genre}
                                            onChange={(e) => updateSetting('genre', e.target.value)}
                                            placeholder="E.g., Minecraft Gameplay, Medical Lecture, 19th Century Drama... (Be specific for better tone)"
                                            className="w-full bg-slate-800 border border-slate-700 rounded-lg py-3 px-4 text-slate-200 focus:outline-none focus:border-indigo-500 text-sm placeholder-slate-600 shadow-inner"
                                            autoFocus
                                        />
                                    </div>
                                )}
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        >
                                            ...
                                            <span>Bilingual (Original + CN)</span>
                                        </button>
                                        <button
