...
export const SubtitleEditor: React.FC<SubtitleEditorProps> = React.memo(({
    ...
    const hasActiveFilter = filters.duration || filters.length || filters.overlap || filters.speakers.size > 0;

    // Filter subtitles based on filters
    const filterByType = React.useCallback((subs: SubtitleItem[]): SubtitleItem[] => {
        ...
        return subs.filter((sub, index) => {
            ...
            const hasMatchingSpeaker = speakerFilterActive && sub.speaker && filters.speakers.has(sub.speaker);
