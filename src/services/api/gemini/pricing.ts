...
export const GEMINI_PRICING: Record<string, ModelPricing> = {
  ...
  'gemini-3-pro-preview': {
    textInput: 2.0,
    audioInput: 2.0, // Same as text for Pro
    textInputHighContext: 4.0,
    audioInputHighContext: 4.0,
    output: 12.0,
    outputHighContext: 18.0,
    cacheText: 0.2,
    cacheAudio: 0.2,
    cacheTextHighContext: 0.4,
    cacheAudioHighContext: 0.4,
  },
...
export const calculateDetailedCost = (usage: {
  ...
  const textRate =
    isHighContext && pricing.textInputHighContext
      ? pricing.textInputHighContext
      : pricing.textInput;
  const audioRate =
    isHighContext && pricing.audioInputHighContext
      ? pricing.audioInputHighContext
      : pricing.audioInput;
  const outputRate =
    isHighContext && pricing.outputHighContext ? pricing.outputHighContext : pricing.output;
  const cacheTextRate =
